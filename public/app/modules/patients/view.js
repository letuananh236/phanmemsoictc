...
function PatientsPage({ onLicenseExpired }) {
  ...
  const handleSave = async () => {
    ...
    setError('');
    try {
      const payload = { name: form.name, gender: form.gender, dob: form.dob, phone: form.phone, address: form.address };
      const saved = form.id ? await api.updatePatient(form.id, payload) : await api.createPatient(payload);
      setForm({
        id: saved.id,
        name: saved.name || '',
        gender: saved.gender || '',
