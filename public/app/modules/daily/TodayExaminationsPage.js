...
function TodayExaminationsPage({ initialDate, onNavigateExam, onCreatePatient, onLicenseExpired }) {
  ...
  const fetchExams = async (opts = {}) => {
    ...
    setError('');
    try {
      const res = await api.listExaminations({ date, doctorId, search: searchText, ...opts });
  ...
  return h(
    ...
    h(
      ...
      h('div', null, h('h2', null, 'Khám trong ngày'), h('p', { className: 'muted' }, 'Lọc nhanh và vào khám ngay cho các phiếu trong ngày.'))
    ),
    h(FilterBar, {
      date,
      doctorId,
      ...
      h(
        ...
        h(
          'button',
          {
            ...
            type: 'button',
            disabled: !selected,
            onClick: () => selected && onNavigateExam?.(selected)
          },
          'VÀO KHÁM'
        )
      )
