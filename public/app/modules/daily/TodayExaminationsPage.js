...
function FilterBar({ date, doctorId, searchText, doctors, onChange, onRefresh, loading }) {
  return h(
    ...
    h(
      ...
      h('input', {
        type: 'search',
        placeholder: 'Tên BN / SĐT / Mã phiếu',
        value: searchText,
...
function ExaminationTable({ exams, selectedId, onSelect, onOpen, loading }) {
  ...
  if (!exams.length) {
    return h('div', { className: 'empty-state' }, 'Không tìm thấy phiếu khám phù hợp.');
  }
...
function TodayExaminationsPage({ initialDate, onNavigateExam, onCreateExam, onLicenseExpired }) {
  ...
  return h(
    ...
    h(
      ...
      h(
        'div',
        null,
        h('h2', null, 'Tìm phiếu khám'),
        h('p', { className: 'muted' }, 'Tra cứu phiếu khám theo ngày, bác sĩ hoặc từ khóa để mở và tiếp tục xử lý.')
      )
    ),
    h(FilterBar, {
      date,
    ...
    h(
      ...
      h('div', { className: 'muted' }, 'Double click dòng hoặc chọn và bấm "Vào khám" để mở chi tiết.'),
      h(
        'div',
        { className: 'actions-row' },
        h(
          'button',
          { className: 'btn secondary', type: 'button', onClick: () => onCreateExam?.() },
          'TẠO PHIẾU KHÁM MỚI'
        ),
        h(
          'button',
