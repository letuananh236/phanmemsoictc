...
function Field({ label, required, children, className }) {
  const classes = ['field'];
  if (required) classes.push('required-field');
...
function PatientBlock({ patient, onChange, errors }) {
  ...
  const visitStatus = patient?.visitStatus === 'returning' ? 'returning' : 'new';
  return h(
    Section,
    { title: 'Thông tin bệnh nhân' },
    h(
      'div',
      { className: 'patient-row primary' },
      h(Field, { label: 'Mã số BN', className: 'short', required: true }, h('input', { value: patient?.code || '', disabled: true })),
      h(Field, { label: 'Họ tên BN', required: true, className: 'grow' },
        h('input', {
          value: patient?.fullName || '',
          oninput: (e) => onChange({ fullName: e.target.value })
        })
      ),
      h(Field, { label: 'Tuổi', required: true, className: 'tiny' },
        h('input', {
          type: 'number',
          min: 0,
          value: age || '',
    ...
    h(
      'div',
      { className: 'patient-row secondary' },
      ...
      h(Field, { label: 'Địa chỉ', className: 'wide' },
        h('input', {
    ...
    h(
      'div',
      { className: 'patient-row full' },
      h(Field, { label: 'Lý do khám bệnh', className: 'full' },
        h('textarea', {
          rows: 3,
...
function ExaminationPage({ examId, onLicenseExpired, onBackToSearch, onOpenCamera, onAddDoctor, license, refreshKey }) {
    ...
    async function loadData() {
      ...
      try {
        ...
        setPatient({
          ...
          visitStatus: detail.patient?.visitStatus || detail.exam?.visitStatus || 'new'
        });
  ...
  const handleSave = async (alsoPrint = false) => {
    ...
    try {
      const payload = {
        ...
        reason: patient?.reason || exam?.reason,
  ...
  if (!examId) {
    return h(
      ...
      h('p', null, 'Chưa chọn phiếu khám. Vui lòng mở từ màn “Khám trong ngày”.'),
      h('button', { className: 'btn primary', type: 'button', onClick: onBackToSearch }, 'Quay lại Tìm phiếu khám')
    );
  }
      ...
      : h(
          ...
          h(SectionHeading, {
            ...
            actions: h('div', { className: 'heading-actions' },
              h('button', { className: 'btn secondary', type: 'button', onClick: onBackToSearch }, '↩ Quay lại Tìm phiếu'),
