...
function Content({ exam, patient, doctor, config }) {
  ...
  return h(
    ...
    h('div', { className: 'info-grid' },
      h(InfoRow, { label: 'Mã số BN', value: patient?.id || patient?.PatientID }),
      h(InfoRow, { label: 'Số bệnh án', value: exam?.examNumber || exam?.ExamNumber || exam?.id }),
      h(InfoRow, { label: 'Họ và tên', value: patient?.name || patient?.FullName }),
...
export function PrintExaminationPage({ examId }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [configRes, examRes] = await Promise.all([api.getConfig(), api.getExamination(examId)]);
        if (!mounted) return;
        setConfig(configRes);
        setExam(examRes);
        const patientRes = examRes?.patientId ? await api.getPatient(examRes.patientId) : null;
        const doctorRes = examRes?.doctorId ? (await api.listDoctors()).find((d) => d.id === examRes.doctorId) : null;
        if (!mounted) return;
        setPatient(patientRes || null);
        setDoctor(doctorRes || (examRes?.doctorName ? { name: examRes.doctorName } : null));
      } catch (err) {
        console.error(err);
