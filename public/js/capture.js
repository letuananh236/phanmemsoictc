...
export function createCaptureView(appState) {
  ...
  async function openCameraSettings(selectEl) {
    ...
    const resetToDefaults = () => {
      ...
      saveCameraSettings(defaultSettings);
      const deviceId = selectEl.value;
      startCamera(deviceId, defaultSettings);
      showToast('Đã khôi phục cài đặt camera mặc định');
    };
  ...
  return {
    render(target) {
      const captureHotkey = (appState.settings?.captureHotkey || 'F2').toUpperCase();
