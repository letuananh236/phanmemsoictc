...
async function request(path, options = {}) {
  const headers = {
    ...(options.headers || {}),
    ...(currentUser
      ? { 'x-user': currentUser.username || '', 'x-role': currentUser.role || '' }
      : {})
  };
...
export const storage = {
  ...
  uploadImageToPath: (dataUrl, targetPath) => request('/api/images/custom', {
    method: 'POST',
    headers: JSON_HEADERS,
  ...
  sendActivationRequest: async (payload) => request('/api/license/send', {
    method: 'POST',
    headers: JSON_HEADERS,
    body: JSON.stringify(payload)
  }),
