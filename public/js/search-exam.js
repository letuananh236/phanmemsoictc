  ...
  function renderDetail(panel, exam, patient) {
    ...
    panel.innerHTML = `
      ...
      <div class="detail-grid">
        ...
        <div><strong>Chẩn đoán lâm sàng:</strong> ${patient?.reason || ''}</div>
        <div><strong>Chỉ định:</strong> ${patient?.indication || ''}</div>
        <div><strong>Bác sỹ chỉ định:</strong> ${patient?.assignedDoctor || ''}</div>
