...
export function createExamFormView(appState) {
  ...
  function getSpecialistDoctors() {
    return doctors.filter((doctor) => doctor.isSpecialistDoctor !== false);
  }

  function getAssignedDoctors() {
    return doctors.filter((doctor) => doctor.isAssignedDoctor === true);
  }
  ...
  function populateDoctors(select, hiddenInput, selectedId, fallbackName) {
    ...
    getSpecialistDoctors().forEach((doctor) => {
    ...
    const specialistDoctors = getSpecialistDoctors();
    const matchedById = specialistDoctors.find((doctor) => doctor.id === desiredId);
    const matchedByName = specialistDoctors.find((doctor) => doctor.name === desiredName);

    if (matchedById) {
      select.value = matchedById.id;
    } else if (matchedByName) {
      select.value = matchedByName.id;
    } else if (specialistDoctors[0]) {
      select.value = specialistDoctors[0].id;
    }
  ...
  function populateAssignedDoctors(select, input) {
    if (!select) return;
    const assignedDoctors = getAssignedDoctors();
    ...
    const matchedByName = assignedDoctors.find((doctor) => doctor.name === desiredName);
    if (matchedByName) {
      select.value = matchedByName.id;
    }
              ...
              <div class="card patient-card">
                ...
                <div class="patient-grid">
                  ...
                  <div class="field assigned-doctor">
                    ...
                    <input type="hidden" name="patientAssignedDoctor" />
