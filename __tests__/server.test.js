...
test('gửi mã kích hoạt sử dụng form-urlencoded và đính kèm mã máy', async () => {
  ...
  const { server, port } = await startServer();
  try {
    const response = await fetch(`http://localhost:${port}/api/license/send`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ machineId: 'MACHINE-123' })
    });

    const body = await response.json();
    const forwarded = recorded[0];
    const sentBody = forwarded?.options?.body instanceof URLSearchParams
      ? forwarded.options.body.toString()
      : typeof forwarded?.options?.body === 'string'
        ? forwarded.options.body
        : '';

    assert.equal(response.status, 200);
    assert.equal(body.success, true);
    assert.ok(forwarded?.url.includes('script.google.com'));
    assert.ok(sentBody.includes('key=NEW_SECRET_456'));
    assert.ok(sentBody.includes('machineId=MACHINE-123'));
    assert.ok(sentBody.includes('source=app'));
    assert.equal(
      forwarded?.options?.headers?.['Content-Type'],
...
test('tự động sinh mã bác sỹ với tiền tố BS và tăng dần', async () => {
  ...
  try {
    ...
    await fetch(`http://localhost:${port}/api/data/clear`, { method: 'POST' });
