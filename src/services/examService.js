...
export function getExam(examId, dbInstance = getDb()) {
  const db = dbInstance;
  const row = db
    .prepare(
...
export function getExamDetail(examId, dbInstance = getDb()) {
  const db = dbInstance;
  const row = db
    .prepare(
...
export async function saveExam(payload) {
  ...
  const fallbackPatient = payload.patientId ? getPatient(payload.patientId) : null;
  const patientRowId = patientRecord?.rowId || fallbackPatient?.rowId || payload.patientRowId || patientPayload.rowId;
  if (!patientRowId) throw new Error('patient_required');

  const { visitRowId } = runInTransaction((db) => {
    const existing = getExam(examId, db);
    const record = {
      ma_phieu: examId,
  ...
  if (images.length) {
    ...
    for (let index = 0; index < images.length; index += 1) {
      ...
      if (img?.dataUrl || img?.dataURL || img?.base64) {
        const data = img.dataUrl || img.dataURL || img.base64;
        await saveExamImage(examId, index + 1, data, img.note);
      } else if (img?.path) {
        const db = getDb();
        db.prepare(
