...
export function getPatient(idOrCode) {
  const db = getDb();
  const row = db
...
function insertVisitForPatient(patientRowId, doctorId, dateIso = new Date().toISOString().slice(0, 10), note = '') {
  const db = getDb();
...
export function savePatient(payload) {
  ...
  const existing = getPatient(patientCode);
  const record = {
    ma_bn: patientCode,
    ho_ten: payload.name,
  ...
  if (existing) {
    ...
    return { patient: getPatient(patientCode) };
  }
  ...
  const createdPatient = getPatient(patientCode);
  let createdVisit = null;
  if (payload.createVisit) {
    createdVisit = insertVisitForPatient(createdPatient.rowId, payload.doctorId, payload.visitDate, payload.reason);
  }
