...
async function newExam(req, res) {
  ...
  res.render('exam/form', { exam: null, patient: null, doctors, images: [], error: null });
}
...
async function createExam(req, res) {
  try {
    ...
    console.error(err);
    const doctors = await doctorService.listActive();
    res.render('exam/form', { exam: null, patient: null, doctors, images: [], error: 'Không thể lưu phiếu' });
  }
}
...
async function editExam(req, res) {
  ...
  res.render('exam/form', { exam: detail.exam, patient: detail.patient, doctors: detail.doctorList, images: detail.images, error: null });
}
...
async function updateExam(req, res) {
  ...
  try {
    ...
    console.error(err);
    const detail = await examService.getExamDetail(id);
    res.render('exam/form', { exam: detail.exam, patient: detail.patient, doctors: detail.doctorList, images: detail.images, error: 'Không thể cập nhật' });
  }
}
