...
export async function handleApi(req, res, pathname) {
  ...
  try {
    ...
    if (normalized === '/visits' || normalized === '/examinations' || normalized === '/exams') {
      if (req.method === 'GET') {
        const query = Object.fromEntries(requestUrl.searchParams.entries());
        const filters = {
      ...
      if (req.method === 'POST') {
        const body = await parseBody(req);
        const saved = await saveExam(body);
        sendJson(res, 201, saved);
        return;
      }
    }

    if (normalized.startsWith('/visits/') || normalized.startsWith('/examinations/') || normalized.startsWith('/exams/')) {
      ...
      if (parts.length === 2) {
        ...
        if (req.method === 'PUT') {
          const body = await parseBody(req);
          const saved = await saveExam({ ...body, id });
      ...
      if (parts[2] === 'print-data') {
        const payload = getPrintPayload(id);
        if (!payload) {
          sendJson(res, 404, { error: 'not_found' });
          return;
        }
    ...
    if (normalized === '/visit-images') {
      if (req.method === 'GET') {
        const visitId = requestUrl.searchParams.get('visitId');
        if (!visitId) {
          sendJson(res, 400, { error: 'missing_visit' });
          return;
        }
        sendJson(res, 200, listVisitImages(visitId));
        return;
      }
      if (req.method === 'POST') {
        const body = await parseBody(req);
        try {
          const saved = await saveVisitImage(body.visitId, body.dataUrl, body.displayOrder, body.note);
          sendJson(res, 201, saved);
        } catch (err) {
          const status = err?.status || 500;
          sendJson(res, status, { error: err?.message || 'upload_failed' });
        }
        return;
      }
    }

    if (normalized.startsWith('/visit-images/by-visit/')) {
      const visitId = decodeURIComponent(normalized.split('/').pop());
      if (req.method === 'DELETE') {
        deleteVisitImagesByVisit(visitId);
        sendJson(res, 200, { success: true });
        return;
      }
    }

    if (normalized.startsWith('/visit-images/')) {
      const id = decodeURIComponent(normalized.split('/').pop());
      if (req.method === 'DELETE') {
        deleteVisitImage(Number(id));
        sendJson(res, 200, { success: true });
        return;
      }
    }
    ...
    if (normalized === '/doctors/active') {
      if (req.method === 'GET') {
        sendJson(res, 200, listDoctors({ activeOnly: true }));
        return;
      }
    }
