...
import { saveImageFromDataUrl, saveImageFromDataUrlToTarget, saveLogoFile } from './models/image.model.js';
...
function createApp() {
  ...
  app.post('/api/license/send', async (req, res) => {
    ...
    const machineKey = String(machineId || generateMachineKey() || '');
    const form = new URLSearchParams();
    form.set('key', ACTIVATION_SECRET);
    form.set('machineId', machineKey);
    form.set('source', 'app');
    try {
      ...
      res.json({ success: true, detail: detail?.trim() || undefined });
    } catch (error) {
  ...
  app.post('/api/images/custom', async (req, res) => {
    try {
      const storedPath = await saveImageFromDataUrlToTarget(req.body || {});
      res.json({ path: storedPath });
    } catch (error) {
      console.error('Không thể lưu ảnh theo đường dẫn chỉ định', error);
      res.status(400).json({ error: 'invalid_image_target' });
    }
  });
