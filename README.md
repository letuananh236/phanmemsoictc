...
## Chức năng chính
- Đăng nhập (mặc định `admin` / `123456@`), kiểm tra bản quyền theo mã máy và hiển thị ngày sử dụng còn lại.
...
- Chạy thử giao diện desktop: `npm run electron:start` (Electron 39.2.4)
- Đóng gói bộ cài Windows: `npm run electron:build` (tạo file .exe bằng Electron Builder với Electron 39.2.4)

Khi chạy bản Electron, dữ liệu (SQLite, ảnh, logo) được lưu trong thư mục dữ liệu của ứng dụng (`%APPDATA%/PhanMemSoICTC/database` hoặc tương đương trên macOS/Linux) thay vì thư mục dự án.
